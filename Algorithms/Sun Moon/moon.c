/*
 * $Id: moon.c,v 1.1.1.1 1993/08/17 09:42:31 alm Exp $
 */
/*
 * moon.c
 *
...
#define prt(y) XDrawImageString(mydisplay, drawable, gcm, info_col,\
	charhgt*(y), tbuf, strlen(tbuf))
#define prtxy(x,y) XDrawImageString(mydisplay, drawable, gcm, charwid*(y+1),\
...
long seconds;
{
	...
	XSetForeground(mydisplay, gcms, WhitePixel(mydisplay, screen));
	XSetBackground(mydisplay, gcms, BlackPixel(mydisplay, screen));
	XDrawImageString(mydisplay, icon_mpr, gcms,
		(64 - txt_size) / 2, 63,
		tbuf, strlen(tbuf));
